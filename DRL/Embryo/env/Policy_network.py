...
AI_SPEED = AI_CELL_SPEED_PER_MIN / float(TICK_RESOLUTION)
FACTOR = 1/2**0.5
ACTIONS = AI_SPEED * (np.array([[1,0,0],[FACTOR,FACTOR,0],
...
class Net_H(nn.Module):
    def __init__(self, ):
        ...
        self.fc3 = nn.Linear(1024, 1024)
        self.fc3.weight.data.normal_(0, 0.1)   # initialization                
        self.out = nn.Linear(1024, N_ACTIONS_H)
        self.out.weight.data.normal_(0, 0.1)   # initialization
...
class CNN_Net(nn.Module):
    def __init__(self, ):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(N_CHANNEL * N_INPUT, 16, 5, 5, 2),
            nn.ReLU(),
            nn.MaxPool2d(2),
            )                               #output (32x16x16)
        ...
        self.out = nn.Linear(32*8*8, N_ACTIONS)
...
class DQN_H(object):
    def __init__(self):
        if use_cuda:
            self.eval_net, self.target_net = Net_H().cuda(), Net_H().cuda()
        else:
            self.eval_net, self.target_net = Net_H(), Net_H()
        self.e_greedy = EPSILON
        self.learning_rate = LR
        self.learn_step_counter = 0                                     # for target updating
        self.memory_counter = 0                                         # for storing memory
